...
def train(train_dataset,val_dataset,test_dataset,model,output_path,train_neg_samp_ratio,val_neg_samp_ratio,
            learning_rate,batch_size,warmup_ratio,num_epochs,evaluator,train_loss,margin,
    ...
    if not isinstance(train_loss, MultipleNegativesRankingLoss):
    ...
    generate_negative_pairs(val_dataset, os.path.join(dataset_output_dir, 'val_dataset_wneg.csv'), val_neg_samp_ratio, "0")
    val_df = pd.read_csv(os.path.join(dataset_output_dir, 'val_dataset_wneg.csv'))
    val_sent_1 = val_df[class_col].tolist()
    val_sent_2 = val_df[text_col].tolist()
    val_labels = val_df["Labels"].tolist()
    val_sim_func = SimilarityFunction.EUCLIDEAN
    evaluator = evaluator(val_sent_1, val_sent_2, val_labels, write_csv=True, show_progress_bar=True, main_similarity=val_sim_func)
    ...
    with open(os.path.join(output_path, 'training_params.csv'), 'w', newline='') as file:
        ...
        # Write the header
        writer.writerow(["Trained Model", "Pretrained on", "No. Shots", "Training Loss",
                        "Margin","Evaluator","Train Neg Ratio","Val Neg Ratio","Learning Rate",
                        "Batch Size", "Warmup Ratio", "Num Epochs", "Random State"])

        # Write the data
        writer.writerow([trained_model,model,shots, str(train_loss)+ "("+str(train_sim_func)+")",margin,str(evaluator)+"("+str(val_sim_func)+")",

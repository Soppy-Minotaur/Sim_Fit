...
class SentenceAccuracyEvaluator(SentenceEvaluator):
    """
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        logger.info("SentenceAccuracyyEvaluator: Evaluating the model on " + self.name + " dataset" + out_txt)
        ...
        accuracy = torch.mean((predictions == index_labels_tensor).float())

        logger.info(
            "Sentence Micro Accuracy :\Accuracy: {:.4f}".format(accuracy)
        )
            ...
            with open(csv_path, newline="", mode="a" if output_file_exists else "w", encoding="utf-8") as f:
                ...
                writer.writerow(
                    [
                        epoch,
                        steps,
                        accuracy.item() # convert to float
                    ]
                )
